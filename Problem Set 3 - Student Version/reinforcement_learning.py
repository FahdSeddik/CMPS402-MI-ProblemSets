...
class SARSALearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, next_action: Optional[A]):
        ...
        next_action = next_action if next_action is not None else env.actions()[0]
        # Q(s, a) = Q(s, a) + alpha * (reward + gamma * Q(s', a') - Q(s, a))
        self.Q[state][action] += self.learning_rate * \
            (reward + self.discount_factor * self.Q[next_state][next_action] - self.Q[state][action]) # SARSA update rule
...
class QLearningAgent(RLAgent[S, A]):
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        next_q = 0 if done else max(self.compute_q(env, next_state, a) for a in env.actions())
...
class ApproximateQLearningAgent(RLAgent[S, A]):
    ...
    def __compute_q_from_features(self, features: Dict[str, float], action: A) -> float:
        ...
        return sum(w * features[feature] for feature, w in self.weights[action].items())
    ...
    def update(self, env: Environment[S, A], state: S, action: A, reward: float, next_state: S, done: bool):
        ...
        next_q = 0 if done else self.__compute_utility_from_features(next_features)
        q = self.__compute_q_from_features(features, action)
        for feature in features:
            self.weights[action][feature] += self.learning_rate * (reward + self.discount_factor * next_q - q) * features[feature]
